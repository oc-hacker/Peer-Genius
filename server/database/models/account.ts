...
const blockUserEdit = (instance: AccountInstance) => {
  if (instance.changed("userId")) {
    throw new ProhibitedEditError(
      "Editing the user FK of accounts table is prohibited."
    );
  }
};
...
const model: Sequelize.Model<AccountInstance, AccountAttributes> = admin.define<
  AccountInstance,
  AccountAttributes
>("accounts", attributes, {
  charset: "utf8"
